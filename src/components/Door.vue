<template>
    <div class="door-area">
        <div class="door-frame" :class="{ selected: selected && !open }"><!-- ele só vai aplicar o selected no door frame se ele tiver selecionado e se não estiver aberta (a classe css que quero aplicar e a expresão que irar dizer se vai ser aplicado ou não ) -->
        ...
        <div class="door" :class="{ open }"
            ...
            <div class="knob" :class="{ selected }"
                @click.stop="open = true" :load="log(open)"></div><!-- .stop, para a propagação do @click de fora -->
        </div>
    </div>
</template>
